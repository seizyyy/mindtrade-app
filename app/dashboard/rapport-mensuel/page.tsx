      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10, marginBottom: 20 }}>
            {[
              { label: "P&L Net", value: `${pnlNet >= 0 ? "+" : ""}${pnlNet.toFixed(0)}${sym(currency)}`, color: pnlNet > 0 ? "var(--g)" : pnlNet < 0 ? "var(--r)" : "var(--ink3)", sub: accountSize ? `${pnlNet >= 0 ? "+" : ""}${((pnlNet / accountSize) * 100).toFixed(2)}%` : `${wins}W / ${losses}L` },
                    ...
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <div style={{ flex: 1, height: 28, background: "var(--bg2)", borderRadius: 6, overflow: "hidden", position: "relative" }}>
                        ...
                        <span style={{ position: "absolute", left: "50%", top: "50%", transform: "translate(-50%,-50%)", fontSize: 12, fontWeight: 700, color: row.val >= 0 ? row.color : "var(--r)", whiteSpace: "nowrap" }}>
                          {row.val >= 0 ? "+" : ""}{row.val.toFixed(0)}{sym(currency)} / jour
                        </span>
                      </div>
                    </div>
                  );
                    ...
                    return (
                      <div key={i}>
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                          ...
                          <span style={{ fontSize: 11, color: "var(--ink3)" }}>{stats.count} trade{stats.count > 1 ? "s" : ""} · {pct}% · <span style={{ color: stats.pnl >= 0 ? "var(--g)" : "var(--r)", fontWeight: 700 }}>{stats.pnl >= 0 ? "+" : ""}{stats.pnl.toFixed(0)}{sym(currency)}</span></span>
                        </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {bestTrade && bestTrade.pnl > 0 && (
                <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: "16px 18px", flex: 1 }}>
                  ...
                  <div style={{ fontFamily: "var(--font-fraunces)", fontSize: 24, color: "var(--g)", fontWeight: 700 }}>+{bestTrade.pnl.toFixed(0)}{sym(currency)}</div>
              ...
              {worstTrade && worstTrade.pnl < 0 && (
                <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: "16px 18px", flex: 1 }}>
                  ...
                  <div style={{ fontFamily: "var(--font-fraunces)", fontSize: 24, color: "var(--r)", fontWeight: 700 }}>{worstTrade.pnl.toFixed(0)}{sym(currency)}</div>
