    ...
    <div style={{ maxWidth: 940, margin: "0 auto" }}>
      ...
      {showForm && (
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: "24px 28px", marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: "var(--ink3)", display: "block", marginBottom: 5 }}>P&L (€)</label>
              ...
              {sorted.map(([emotion, stats]) => {
                ...
                return (
                  <div key={emotion} style={{ background: bg, border: `1px solid ${border}`, borderRadius: 10, padding: "14px 16px" }}>
                    ...
                    <div style={{ fontSize: 11, color: "var(--ink3)", marginTop: 5 }}>{stats.count} trade{stats.count > 1 ? "s" : ""} · {stats.pnl >= 0 ? "+" : ""}{stats.pnl.toFixed(0)}€</div>
                  </div>
                );
              })}
            </div>
